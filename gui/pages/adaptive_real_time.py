...
class ThreadSafeScaleSignaler(QObject):
...
class AdaptiveRealTime(QScrollArea):
    ...
    def create_queue(self):
        import copy
        import random
        trial_count = self.configurations.read('real_time.trial_count')
        pause_length = self.configurations.read('real_time.pause_length')
        trial_length = self.configurations.read('real_time.trial_length')
        labels = self.configurations.read('real_time.labels')
        
        # Use all configured labels
        active_labels = labels
            
        label_queue = list(np.repeat(active_labels, trial_count))
    ...
    def open_prompt_window(self):
        ...
        self.prompt_viewer = PromptViewer(self, self.start_or_stop_acquisition, self.on_prompt_viewer_close, self.progressbar_value, is_adaptive=True)
        self.arrow_scale_signaler = ThreadSafeScaleSignaler(self.prompt_viewer.set_arrow_scale)
    ...
    def real_time_processor(self, current_signal, current_true_label=None):
        ...
        try:
            ...
            if hasattr(self.loaded_pipeline, "predict_proba"):
                ...
                # Dynamic scaling using target label
                if current_true_label is not None:
                    classes_norm = [str(c).lower() for c in classes]
                    if str(current_true_label).lower() in classes_norm:
                        idx = classes_norm.index(str(current_true_label).lower())
                        target_prob = probs[idx]
                
            if hasattr(self, 'arrow_scale_signaler'):
                self.arrow_scale_signaler(float(target_prob))
            ...
            # Trial extraction and refitting
            if hasattr(self, 'current_queue') and len(self.current_queue) > 0:
                current_item = self.current_queue[0]
                batches_per_second = 10
                # Check if this item is in its last batch
                if current_item[1] <= (1.0 / batches_per_second) + 0.05:
                    label = current_item[0]
                    # Only collect for active classes (including rest, excluding break)
                    if label != 'break':
                        self.X_history.append(filtered_data) # store (channels, times)
                        self.y_history.append(label)
                        self._update_classifier()
                
            return None # Return None so PromptViewer shows true label from queue
        except Exception as e:
            return None

    def _update_classifier(self):
        if not hasattr(self, 'X_history') or len(self.X_history) < 2:
            return
            
        unique_classes = np.unique(self.y_history)
        config_labels = self.configurations.read("real_time.labels")
            
        # Ensure we have at least one of each active class
        if len(unique_classes) < len(config_labels):
            return
            
        try:
            X_arr = np.array(self.X_history)
            y_arr = np.array(self.y_history)
            if hasattr(self.loaded_pipeline, 'partial_fit'):
                self.loaded_pipeline.partial_fit(X_arr, y_arr)
                print(f"Classifier partial_fit successfully on {len(y_arr)} trials.")
            else:
                self.loaded_pipeline.fit(X_arr, y_arr)
                print(f"Classifier fit successfully on {len(y_arr)} trials.")
        except Exception as e:
