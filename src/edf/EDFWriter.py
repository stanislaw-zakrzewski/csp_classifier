...
class EDFWriter:
    ...
    def write(self, signal, start_date, queue, patient_name, gender, channels_in_order):
        t = time.localtime()
        timestamp = time.strftime('%Y-%m-%dT%H-%M-%S', t)
        filename = 'data/{}.edf'.format(timestamp)

        sig_headers = highlevel.make_signal_headers(channels_in_order, sample_rate=self.sampling_frequency,
                                                    physical_max=1000.0,
                                                    physical_min=-1000.0)
        ...
        for channel_signal in signal:
            clipped_singal.append(np.clip(channel_signal, -1000.0,1000.0))
            mins.append(np.min(clipped_singal[-1]))
            maxes.append(np.max(clipped_singal[-1]))
        # tutaj filtrujemy
        highlevel.write_edf(filename, clipped_singal, sig_headers, header)
