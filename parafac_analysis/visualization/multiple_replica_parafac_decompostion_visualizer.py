...
with open(filename, 'rb') as out_file:
    ...
    available_ranks = sorted(dataframe['rank'].unique())

    print('Available ranks for replica decomposition visualization:')
    for available_rank in available_ranks:
        print("\t{}".format(available_rank))
    selected_rank = int(input('Selected rank: '))

    dataframe = dataframe.loc[dataframe['rank'] == selected_rank]

    df2 = dataframe.groupby(['replica']).size().reset_index(name='statistically significant atoms')
    ...
    sns.lineplot(dataframe, x='replica', y='pvalue', ax=ax0)
    ax0.set_yscale('log')
    ax1.set_yscale('linear')

    # ax12 = ax1.twinx()
    sns.lineplot(data=df2, x='replica', y='statistically significant atoms', ax=ax1, color='red')
    ax1.set_ylim([0, None])
    ax0.xaxis.grid(True)
    ax0.xaxis.set_major_locator(ticker.MultipleLocator(1))

    sns.lineplot(dataframe, x='replica', y='weight', ax=ax2)
    ax2.set_yscale('log')
    ax3.set_yscale('linear')

    # ax22 = ax2.twinx()
    sns.lineplot(data=df2, x='replica', y='statistically significant atoms', ax=ax3, color='red')
    ax3.set_ylim([0, None])
    ax2.xaxis.grid(True)
    ax2.xaxis.set_major_locator(ticker.MultipleLocator(1))

    fig.suptitle(f'Quality of PARAFAC rank {selected_rank} decomposition for:\n{filename}', fontsize=16)
    plt.show()
