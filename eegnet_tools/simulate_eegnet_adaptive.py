        ...
        try:
            ...
            if os.path.exists(test_split_npz):
                ...
                if matching_key in pretrained_models:
                    ...
                    df_strat_c = simulate_eegnet_online_adaptation(X_te, labels_te, strat_c_models, args.dataset, target_subject, adapt_interval=args.adapt_interval, device=args.device, adapt_mode=args.adapt_mode, max_buffer=args.max_buffer)
            ...
            df_full = simulate_eegnet_online_adaptation(X, labels, target_models, args.dataset, target_subject, adapt_interval=args.adapt_interval, device=args.device, adapt_mode=args.adapt_mode, max_buffer=args.max_buffer)

            # Combine Strategy C test split results if evaluated separately
            out_file = os.path.join("simulation_results", "eegnet", args.dataset, f"{target_subject}.csv")
