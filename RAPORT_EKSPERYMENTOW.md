...
### Opis
Konstruuje rozmaitości Riemannian Covariance Tangent Space oraz grafy funkcjonalnej afinitywności badanych badanych (subjects) w zbiorach danych BCI. Trenuje sieć grafową Graph Neural Network (GNN) z funkcją straty Deep Embedded Clustering (DEC) ($L_{DEC} = KL(P \parallel Q)$), aby pogrupować dawców (donors) w 5 topologicznie dopasowanych podpopulacji.
...
- **Dane Wejściowe**: Surowe próby EEG z 7 zbiorów danych MOABB (`Dreyer2023`, `Dreyer2023A`, `PhysionetMI`, `Lee2019_MI`, `GuttmannFlury2025_MI`, `GuttmannFlury2025_ME`, `Yang2025`).
...
- **Porównywany Z**: Klasycznymi potokami **CSP + LDA** oraz **Covariance Tangent Space + Logistic Regression**.
- **Dane Wyjściowe Do**: Dostarcza bazową architekturę dla **Eksperymentu 3** (ATCNet) oraz **Eksperymentu 4** (Head-Only Adaptation).
...
### Powiązane Eksperymenty
- **Powiązany Z**: Wykorzystuje klastry GNN z **Eksperymentu 1**.
- **Porównywany Z**: PyTorch EEGNet (**Eksperyment 2**) oraz Klasycznymi Baseline'ami.
...
| **Exp 6: Population Transfer** | **N - 5 Dawców (100+)** | **Population ATCNet** | **69.11%** | **61.67%** | 0 Prób |
